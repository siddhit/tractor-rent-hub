...
const WhatsAppButton = () => {
  ...
  return (
    ...
    >
      {/* WhatsApp icon glyph */}
      <span className="w-7 h-7 rounded-full bg-cream text-monsoon flex items-center justify-center font-black text-sm flex-shrink-0">
      ...
      <div className="flex flex-col items-start leading-tight">
        <span className={language === 'gu' ? 'font-gujarati' : ''}>{t('wa.pill')}</span>
        <span className="text-xs font-mono opacity-80">{t('wa.sub')}</span>
      </div>
    </a>
  );
};
